...
class twoSEBasicBlock(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, r=16):
        super(twoSEBasicBlock, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        if groups != 1 or base_width != 64:
            raise ValueError('BasicBlock only supports groups=1 and base_width=64')
        ...
        self.bn1 = norm_layer(planes)
        self.relu = nn.ReLU(inplace=True)
        self.se1 = SEBlock(planes, r)
        self.conv2 = conv3x3(planes, planes)
        self.bn2 = norm_layer(planes)
        self.downsample = downsample
        self.stride = stride
        # add SE block
        self.se2 = SEBlock(planes, r)
...
def twose_resnet18(pretrained=False, progress=True, **kwargs):
    return _resnet('resnet18', twoSEBasicBlock, [2, 2, 2, 2], pretrained, progress,
                   **kwargs)